...
def set_event_confirmation(request, pk):
    ...
    if is_confirmed is not None:
        ...
        event.save()
        return Response({'success': 'Event confirmation status updated successfully'}, status=status.HTTP_200_OK)
    else:
        return Response({'error': 'is_confirmed field is required'}, status=status.HTTP_400_BAD_REQUEST)
